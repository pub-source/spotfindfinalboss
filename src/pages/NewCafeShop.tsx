...
export default function NewCafeShop() {
  ...
  useEffect(() => {
    const channel = supabase
      .channel('cafe-ratings-changes')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'cafe_ratings' }, () => refetchRatings())
      .subscribe();
    return () => { supabase.removeChannel(channel); };
  }, [refetchRatings]);

  const getStats = (id: string) => {
    const rs = ratingsData?.filter(r => r.cafe_id === id) || [];
            ...
            <CardContent className="p-4 flex flex-col flex-1 gap-3">
              <div className="min-h-[3.5rem]">
                ...
                <div className="flex items-center gap-2 text-sm text-muted-foreground mt-1">
                  ...
                  <span className="text-primary line-clamp-1">{cafe.location || 'Location not specified'}</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1 text-sm text-muted-foreground min-h-[1.25rem]">
                {cafe.opening_hours && (
                  <>
                    <Clock className="h-3 w-3" />
                    <span className="line-clamp-1">{cafe.opening_hours}</span>
                  </>
                )}
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-sm font-medium">
                    {(() => { const s = getStats(cafe.id); return `${s.avgRating.toFixed(1)} / ${s.totalReviews} reviews`; })()}
                  </span>
                </div>
                {cafe.price_range && (
                  <span className="text-sm font-medium text-primary">{cafe.price_range.replace(/\$/g, '₱')}</span>
                )}
              </div>
